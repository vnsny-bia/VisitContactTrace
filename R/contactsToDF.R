...
contactsToDF <- function(a){
  ...
  #second_column_frwd <- trimws(sub("^\\s*(\\S+\\S+).*", "\\1", second_column_frwd))
  #second_column_frwd <- trimws(gsub('\\(','',gsub('(clin_.+?.+\\().*','\\1',second_column_frwd,perl = T)))
  ...
  #second_column_bkwd <- trimws(gsub('\\(','',gsub('(clin_.+?.+\\().*','\\1',second_column_bkwd,perl = T)))
